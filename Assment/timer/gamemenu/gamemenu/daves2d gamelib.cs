...
namespace gamelib2d
{
    ...
    public class sprite2d
    {
        ...
        public void automove(int pheight, int bheight,float gtime)
        {
            ...
            if (position.Y >= pheight - rect.Height / 2)
            {
                velocity.Y = -velocity.Y;
                position.Y = pheight - rect.Height / 2;
            }
            if ((position.Y - rect.Height / 2) <bheight)
            {
                velocity.Y = -velocity.Y;
                position.Y = bheight + rect.Height / 2;
            }
            updateobject();
        }


        public void automove2(int lwidth, int rwidth, float gtime)
        {
            // Add code here for when the game is running
            rotation += rotspeed; // Spin Ball
            ...
            if (position.X >= rwidth - rect.Height /2)
            {
                velocity.X = -velocity.X;
                position.X = rwidth - rect.Width /2;
            }
            if ((position.X - rect.Width / 2) <lwidth)
            {
                velocity.X = -velocity.X;
                position.X = lwidth + rect.Width /2;
            }
            updateobject();
        }
        public void automove3(int lwidth, int rwidth, float gtime)
        {
            // Add code here for when the game is running
    ...
    public class animatedsprite
    {
        ...
        public float gravity = 2f;                     // Force of gravity to apply
        public float power = 0.03f;                       // Potential of character for speed
        ...
        public void automove2(int gwidth, int gheight, float gtime)
        {
            // Add code here for when the game is running
            velocity.X += power;
